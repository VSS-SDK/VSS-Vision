...
CalibrationRoutine::CalibrationRoutine(ICalibrationRepository *calibrationRepository, ICalibrationBuilder *calibrationBuilder){
...
bool CalibrationRoutine::on_keyboard(GdkEventKey* event, Gtk::Window* window){
...
void CalibrationRoutine::on_signal_select_dialog(Gtk::FileChooserDialog* file_chooser, Gtk::Entry* entry){
  std::string str = file_chooser->get_filename();
  std::size_t sub_str = str.find_last_of("/\\");
  entry->set_text(str.substr(sub_str+1));
}

void CalibrationRoutine::on_button_save_calibration(Gtk::FileChooserDialog* file_chooser, Gtk::Entry* entry){
...
void CalibrationRoutine::on_button_load_dialog(Gtk::FileChooserDialog* file_chooser, Gtk::Entry* entry){
  entry->set_sensitive(false);
  file_chooser->show();
}

void CalibrationRoutine::on_button_save_dialog(Gtk::FileChooserDialog* file_chooser, Gtk::Entry* entry){
...
void CalibrationRoutine::on_combo_box_color_team2(Gtk::ComboBoxText* color_team_2){
  std::cout << color_team_2->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_color_robot1(Gtk::ComboBoxText* color_robot_1){
  std::cout << color_robot_1->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_color_robot2(Gtk::ComboBoxText* color_robot_2){
  std::cout << color_robot_2->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_color_robot3(Gtk::ComboBoxText* color_robot_3){
  std::cout << color_robot_3->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_color_robot4(Gtk::ComboBoxText* color_robot_4){
  std::cout << color_robot_4->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_color_robot5(Gtk::ComboBoxText* color_robot_5){
  std::cout << color_robot_5->get_active_text() << std::endl;
}

void CalibrationRoutine::on_combo_box_input_path(Gtk::ComboBoxText* input_path){
  std::cout << input_path->get_active_text() << std::endl;
}

void CalibrationRoutine::on_scale_h_max(Gtk::Scale* scale_h_max){
  setColorRangePart(ColorRangePart::H_MAX, scale_h_max->get_value());
}

void CalibrationRoutine::on_scale_h_min(Gtk::Scale* scale_h_min){
  setColorRangePart(ColorRangePart::H_MIN, scale_h_min->get_value());
}

void CalibrationRoutine::on_scale_s_max(Gtk::Scale* scale_s_max){
  setColorRangePart(ColorRangePart::S_MAX, scale_s_max->get_value());
}

void CalibrationRoutine::on_scale_s_min(Gtk::Scale* scale_s_min){
  setColorRangePart(ColorRangePart::S_MIN, scale_s_min->get_value());
}

void CalibrationRoutine::on_scale_v_max(Gtk::Scale* scale_v_max){
  setColorRangePart(ColorRangePart::V_MAX, scale_v_max->get_value());
}

void CalibrationRoutine::on_scale_v_min(Gtk::Scale* scale_v_min){
  setColorRangePart(ColorRangePart::V_MIN, scale_v_min->get_value());
}

void CalibrationRoutine::on_scale_rotation(Gtk::Scale* scale_rotation){
  std::cout << scale_rotation->get_value() << std::endl;
  calibration.rotation = (float)scale_rotation->get_value();
}

void CalibrationRoutine::on_scale_brightness(Gtk::Scale* scale_brightness){
  std::cout << scale_brightness->get_value() << std::endl;
  calibration.brightness = (float)scale_brightness->get_value();
}

void CalibrationRoutine::on_scale_contrast(Gtk::Scale* scale_contrast){
  std::cout << scale_contrast->get_value() << std::endl;
  calibration.contrast = (float)scale_contrast->get_value();

}

void CalibrationRoutine::on_scale_saturation(Gtk::Scale* scale_saturation){
  std::cout << scale_saturation->get_value() << std::endl;
  calibration.saturation = (float)scale_saturation->get_value();
}

void CalibrationRoutine::on_scale_exposure(Gtk::Scale* scale_exposure){
  std::cout << scale_exposure->get_value() << std::endl;
  calibration.exposure = (float)scale_exposure->get_value();
}

void CalibrationRoutine::on_scale_gain(Gtk::Scale* scale_gain){
  std::cout << scale_gain->get_value() << std::endl;
  calibration.gain = (float)scale_gain->get_value();
}

void CalibrationRoutine::on_toggle_button_cut_mode(Gtk::ToggleButton* toggle_button_cut_mode){
    std::cout << toggle_button_cut_mode->get_active() << std::endl;
}

void CalibrationRoutine::on_radio_button_image(Gtk::RadioButton* radio_button_image){
  if (!radio_button_image->get_active())
    std::cout << "Image: " << radio_button_image->get_active() << std::endl;
}
void CalibrationRoutine::on_radio_button_video(Gtk::RadioButton* radio_button_video){
  if (!radio_button_video->get_active())
    std::cout << "Video: " << radio_button_video->get_active() << std::endl;
}
void CalibrationRoutine::on_radio_button_camera(Gtk::RadioButton* radio_button_camera){
...
void CalibrationRoutine::setColorRangePart(ColorRangePart part, double value) {
  for (auto& colorRange : calibration.colorsRange) {
